...
import com.example.whatsapp.presentation.splashscreen.homescreen.ChatListModel
...
class BaseViewModel : ViewModel() {
    ...
    fun GetChatForUser(userId: String,callback: (List<ChatListModel>) -> Unit){
        ...
        chatRef.orderByChild("userId").equalTo(userId)
            .addListenerForSingleValueEvent(object : ValueEventListener{
                ...
                override fun onCancelled(error: DatabaseError) {
                    Log.e("BaseViewModel","Error fetching users chat: ${error.message}")
                    callback(emptyList())
                }
            })
    }
   ...
   private fun loadChatData(){
    ...
    fun addChat(newChat:ChatListModel){
        ...
        if (currentUserId!=null){
            ...
            newChatRef.setValue(chatWithUser).addOnSuccessListener {
                ...
                Log.e("BaseViewModel","Failed to add chat data:${exception.message}")
    ...
    fun fetchLastMessageForChat(
        ...
        chatRef.orderByChild("timestamp").limitToLast(1).addListenerForSingleValueEvent(object :
            ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
